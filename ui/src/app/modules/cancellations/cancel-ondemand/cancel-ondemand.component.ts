...
//  
//
//  Created by Luis Romero on December 5th
...
export class CancelOndemandComponent implements OnInit {
    ...
    public cancellationOndemandForm: FormGroup; 
  ...
  constructor(private formBuilder: FormBuilder,
              ...
              this.cancellationOndemandForm = this.formBuilder.group({ 
  ...
  addItems(items){
    let itemsArray = this.cancellationOndemandForm.get('items') as FormArray;
  ...
  closeModal(){
    ...
    let forms = this.cancellationOndemandForm.get("items") as FormArray
  ...
  search(ev){
    ...
    if(ev.formData.numFactura !== null || ev.formData.numCuenta !== null || ev.formData.numContrato !== null || ev.formData.numIdCliente !== null ){
      ...
      if (ev.formData.numFactura !== '' || ev.formData.numCuenta !== '' || ev.formData.numContrato !== '' || ev.formData.numIdCliente !== '') { 
        ...
        this.cancelOndemandService.search(ev.formData.numFactura, ev.formData.numCuenta, ev.formData.numContrato, ev.formData.numIdCliente).subscribe(
          data => {
  ...
  public clearFormData(ev){
    // console.log(ev);
    this.cancellationOndemandForm.reset()
    this.cancellationOndemandForm = this.formBuilder.group({
  ...
  onSelect({ selected }) {
    ...
    let items = this.cancellationOndemandForm.get("items") as FormArray
    ...
    listSelected.forEach(e=>{
      let currentItem  = (this.cancellationOndemandForm.get("items") as FormArray).at(e);
  ...
  continue(){
    ...
    console.log("Form ", this.cancellationOndemandForm.getRawValue())
    let items = this.cancellationOndemandForm.get('items') as FormArray
